...
\parhead{Bridging the gap between Bayesian inference and causal inference.} Our attempt at developing a causal inference approach to recommendation with Bayesian inference in \Cref{chpt:causal_rec} is only a small step of bringing these two fields together. There are still some fundamental problems that need to be theoretically justified. 
