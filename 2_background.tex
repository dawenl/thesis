...
\]
Since the \gls{KL}-divergence $\textrm{KL}(q\|p)$ is non-negative and equals $0$ only if $q = p$, $a.s.$, $\cL(q, \mb\theta)$ acts as a tight lower-bound that equals $\log p(\mbx \g \mb\theta)$ when we set $q$ to $p$. \gls{EM} algorithm optimizes $\cL(q, \mb\theta)$ by iteratively applying the following E(xpectation) and M(aximization) steps:
