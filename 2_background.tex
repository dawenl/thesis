...
\PP Collaborative filtering analyzes user preferences for items (e.g., books,
...
\KwIn{A set of observed entires in the click matrix $\{y_{ui}: (u, i)\in\cO\}$}
...
\begin{algorithm}
\DontPrintSemicolon % Some LaTeX compilers require you to use \dontprintsemicolon instead 
\KwIn{Click matrix $y_{ui}$, the confidence for clicked $c_1$ and unclicked $c_0$}
\KwOut{A set of user latent factors $\mb\theta_{1:U}$ and item latent factors $\mb\beta_{1:I}$}
%$C \gets \emptyset$\;
Randomly initialize $\mb\theta_{1:U}$, $\mb\beta_{1:I}$\;
\While{not converged}{
