...
\mypar{Matrix factorization for collaborative filtering.} User-item preference
...
\mypar{Collaborative filtering for implicit data.} Weighted
...
\begin{figure}[!tbp]
  \centering
  \subcaption[Exposure MF.]{\includegraphics[width=0.25\textwidth]{./fig/plate_diagram.pdf}\label{fig:plate_diagram}}
  %\hfill
  \subcaption[Exposure MF with \newline exposure covariates.]{\includegraphics[width=0.25\textwidth]{./fig/plate_diagram_side_info.pdf}\label{fig:plate_diagram_side_info}}
...
\mypar{Per-item $\mu_i$.} A direct way to encode exposure is via item
...
\mypar{Text topics or locations as exposure covariates.} 
...
%\mypar{Side information from location.}\\
...
\mypar{Causal inference.} Our work borrows ideas from the field of
...
\mypar{Biased CF models.} Authors have recognized that typical
...
\mypar{Modeling exposure with random graphs.} The user-item
...
\mypar{Exposure in other contexts.} In zero-inflated Poisson regression, a
...
\mypar{Versatile CF models.} As we show in \Cref{sec:modeling_mu},
...
\mypar{Empirical evaluation.} Results comparing ExpoMF to WMF on our four
...
\mypar{Exploratory analysis.} We now explore 
...
\mypar{Study.} We evaluate the empirical performance of Content ExpoMF and report results
...
\mypar{Study.} We train Location ExpoMF following the same procedure as
