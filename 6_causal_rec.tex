...
sources of information (see the definition of observational data in \Cref{chpt:background:sec:exp_obs}): which items each user decided to look at and
...
(similar to ExpoMF \citep{Liang16exposure} from \Cref{chpt:expomf}) first generates each user's
...
% \Cref{chpt:causal_rec:sec:exp}). In particular users favor rating items
...
skipped (disliked). For example, in \Cref{chpt:causal_rec:sec:exp} we analyze a large collection of click data from
...
clicked on item $i$ (liked) or decided to skip the item (disliked). This is the explicit counterpart of the setup in \Cref{chpt:expomf}.
...
of the data described in \Cref{chpt:causal_rec:sec:data}: an exposure model of what the user sees and a click
...
factorization~\citep{salakhutdinov2008probabilistic} in \Cref{chpt:background:sec:mf_cf}. Conditional on being
...
% joint to infer the user preferences $\mb\theta_u$ and item attributes
% $\mb\beta_i$ in the intervention joint.
...
\section{Inference}  \label{chpt:causal_rec:sec:inference}
...
\parhead{Popularity model.} For popularity exposure model $a_{ui} \sim \mathrm{Bernoulli}(\rho_i)$, we obtain the maximum likelihood estimate $\hat{\rho}_i$ by counting the portion of the users who have been exposed to item $i$. The propensity score in this case is fixed across users: 
\begin{equation}
...
The click model is a matrix factorization $y_{ui} \g a_{ui} = 1 \sim \cN(\mb\theta_u^\top \mb\beta_i, \lambda_y^{-1})$.
Following \Cref{chpt:background:sec:mf_cf}, we place a diagonal normal prior on both user preference
$\mb\theta_u \sim \mathcal{N}(\bzero, \lambda_\theta^{-1} \mathrm{I}_K)$ and
item attributes
$\mb\beta_i \sim \mathcal{N}(\bzero, \lambda_\beta^{-1}\mathrm{I}_K)$. To fit the model, we compute the maximum \emph{a posteriori} estimates of the parameters $\mb\theta_u$ and $\mb\beta_i$. Concretely, the objective for the inverse propensity weighted Gaussian matrix factorization model (\Cref{eq:causal-training}) is:
\begin{equation*}
\cL = -\sum_{(u, i)\in \cO} \frac{1}{2\pi_{ui}}(y_{ui} - \mb\theta_u^\top\mb\beta_i)^2 - \frac{\lambda_\theta}{2} \sum_u \|\mb\theta_u \|^2_2 - \frac{\lambda_\beta}{2} \sum_i \|\mb\beta_i \|_2^2,
\label{chpt:causal_rec:eq:gmf_logll}
\end{equation*}
where the propensity score $\pi_{ui}$ can be obtained by either \Cref{chpt:causal_rec:eq:pop_prop} or \Cref{chpt:causal_rec:eq:pf_prop}. The observed set $\cO$ contains all the entries with $a_{ui} = 1$. Similar to \gls{WMF}, we can obtain the following coordinate updates by taking the gradients with respect to $\mb\theta_u$ and $\mb\beta_i$ and setting them to 0:
\begin{equation}
...
\end{equation}
The full algorithm for the inverse propensity weighted Gaussian matrix factorization is summarized in \Cref{chpt:causal_rec:algo:gmf}. Note that this algorithm only includes options for fitting the model causally (\Cref{eq:causal-training}). In \Cref{chpt:causal_rec:sec:exp}, we empirically explore different combinations of the exposure model, prediction method, and fitting procedures.
...
%$C \gets \emptyset$\;
Fit exposure model to compute the propensity score (\Cref{chpt:causal_rec:eq:pop_prop} or \Cref{chpt:causal_rec:eq:pf_prop})\;
Randomly initialize $\mb\theta_{1:U}$, $\mb\beta_{1:I}$\;
\While{not converged}{
  \For{$u \gets 1$ \textbf{to} $U$}{
    Update user factor $\mb\theta_u$ (\Cref{chpt:causal_rec:eq:als_user})
  }
  \For{$i \gets 1$ \textbf{to} $I$}{
    Update item factor $\mb\beta_i$  (\Cref{chpt:causal_rec:eq:als_item})
  }
}
...
\Cref{tab:data} summarizes the important attributes of our four datasets.
...
\parhead{Hyperparameters.} We perform grid search using
...
$\mb\beta_i$ in all settings. For the coordinate updates algorithm in \Cref{chpt:causal_rec:sec:inference}, we declare convergence when the mean
...
We report our studies on all data. We evaluate both the exposure
...
As future work, we can develop similar methodology for implicit data. The main difficulty in implicit data is that we do not know which items a user has been exposed to. The ExpoMF model and its variations we developed in \Cref{chpt:expomf} could help with that.  
